<?php
/**
 * Plugin Name:       ODSI Social
 * Plugin URI:        https://github.com/dekumus/odsi-slp
 * Description:       Profiles, activity feed, groups, connections, notifications and messaging for the ODSI social learning platform.
 * Version:           0.1.0
 ...
 *
 * @return string[] Human readable failure reasons. Empty when supported.
 */
function environment_errors(): array {
	...
	if ( version_compare( PHP_VERSION, MIN_PHP, '<' ) ) {
		$errors[] = sprintf(
			...
			__( 'ODSI Social requires PHP %1$s or newer. This site runs PHP %2$s.', 'odsi-social' ),
			MIN_PHP,
			PHP_VERSION
		);
	}
	...
	if ( version_compare( get_bloginfo( 'version' ), MIN_WP, '<' ) ) {
		$errors[] = sprintf(
			...
			__( 'ODSI Social requires WordPress %1$s or newer. This site runs WordPress %2$s.', 'odsi-social' ),
			MIN_WP,
...
/**
 * Register the bundled PSR-4 autoloader, preferring Composer's when built.
 */
...
/**
 * Boot once every plugin is loaded.
 */
